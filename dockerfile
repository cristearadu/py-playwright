...
RUN printf '%s\n' '#!/usr/bin/env bash' \
    ...
    'fi' \
    'exec pytest "${PYTEST_TESTS:-tests}" -n "${PYTEST_WORKERS:-auto}" --run-browser="${RUN_BROWSER:-chrome}" ${HEADLESS_FLAG} "${MARKER_ARGS[@]}" --alluredir=/allure-results --junitxml=/junit-results/results.xml ${PYTEST_EXTRA_ARGS:-}' \
